...
function applyPreset(preset) {
  ...
  if (preset.favorite) {
    ...
    //TODO Build proper promise
    promise = promise.then(() => {
        if (preset.spotify.startsWith("spotify:playlist") && ! preset.spotify.startsWith("spotify:user")) {
            preset.spotify = "spotify:user:" + preset.spotify
        }
        if (preset.spotify_queue_position) {
            spotify.spotify(coordinator, [preset.spotify_queue_position, preset.spotify])
        } else {
            spotify.spotify(coordinator, ["now", preset.spotify])
        }
    });
   }
  ...
  if (preset.state.toLowerCase() === 'playing') {
